...
public sealed class BackwardTraceEndpointTests(PostgreSqlContainerFixture database)
{
    private const string ValidPassword = "Valid-test-password-42!";

    [Fact]
    public async Task BackwardBottleContainsOilAndOlivesWithRootAndDirectedEventEdges()
    {
        ...
        var graph = await ReadGraphAsync(client, setup.Organization.Id, bottles.Id, factory.RequestCancellationToken);

        // Assert the immediate ancestor first: the one-level mutation must still find oil.
        Assert.Contains(graph.Nodes, node => node.LotId == oil.Id && node.LotNumber == "OIL-001");
        Assert.Contains(graph.Nodes, node => node.LotId == olives.Id && node.LotNumber == "OL-001");
        Assert.Equal(bottles.Id, graph.RootLotId);
    ...
    [Fact]
    public async Task LotWithoutAncestorsReturnsOnlyItsRoot()
    {
        var setup = await CreateAuthorizedSetupAsync(StandardRoleIds.Producer);
    ...
    [Fact]
    public async Task DiamondReturnsSharedAncestorExactlyOnceAndEveryEdgeOnce()
    {
        var setup = await CreateAuthorizedSetupAsync(StandardRoleIds.Producer);
        await using var factory = CreateFactory();
        using var client = factory.CreateClient();
        ...
        var graph = await ReadGraphAsync(client, setup.Organization.Id, d.Id, factory.RequestCancellationToken);

        Assert.Single(graph.Nodes, node => node.LotId == a.Id);
    ...
    public async Task ForeignAndMissingLotIdsReturnIndistinguishable404Responses()
    {
        ...
        client.DefaultRequestHeaders.Add("X-Correlation-Id", "backward-not-found-comparison");
    ...
    public async Task ConfiguredNodeLimitIncludesRootAndRejectsOnlyOversizedGraphs(int maxNodes, HttpStatusCode expected)
    {
        ...
        using var response = await client.GetAsync(TracePath(setup.Organization.Id, c.Id), factory.RequestCancellationToken);
    ...
    [Fact]
    public async Task GraphExcludesForeignAndUnrelatedLotsEventsAndSiblingOutputs()
    {
        var own = await CreateAuthorizedSetupAsync(StandardRoleIds.Producer);
        var foreign = await CreateAuthorizedSetupAsync(StandardRoleIds.Producer);
        await using var factory = CreateFactory();
        using var client = factory.CreateClient();
        await AuthenticateAsync(client, own.Account, factory.RequestCancellationToken);
        var a = await CreateLotAsync(client, own, "A", 10m, "KG", factory.RequestCancellationToken);
        var b = await CreateLotAsync(client, own, "B", 5m, "KG", factory.RequestCancellationToken);
        var sibling = await CreateLotAsync(client, own, "SIBLING", 5m, "KG", factory.RequestCancellationToken);
        await CreateLotAsync(client, own, "UNRELATED", 10m, "KG", factory.RequestCancellationToken);
        var split = await CreateEventAsync(client, own, "SPLIT",
            [new(a.Id, 10m)], [new(b.Id, 5m), new(sibling.Id, 5m)], factory.RequestCancellationToken);

        using var foreignClient = factory.CreateClient();
        await AuthenticateAsync(foreignClient, foreign.Account, factory.RequestCancellationToken);
        var foreignA = await CreateLotAsync(foreignClient, foreign, "A", 10m, "KG", factory.RequestCancellationToken);
        var foreignB = await CreateLotAsync(foreignClient, foreign, "B", 10m, "KG", factory.RequestCancellationToken);
        await CreateEventAsync(foreignClient, foreign, "PROCESS",
            [new(foreignA.Id, 10m)], [new(foreignB.Id, 10m)], factory.RequestCancellationToken);

        var graph = await ReadGraphAsync(client, own.Organization.Id, b.Id, factory.RequestCancellationToken);

        Assert.Equal(2, graph.Nodes.Count);
        AssertNode(graph, a);
        AssertNode(graph, b);
        Assert.Equal(new TraceEdgeResponse(split.Id, "SPLIT", split.OccurredAt, a.Id, b.Id), Assert.Single(graph.Edges));
    }
    ...
    private async Task<TestAccount> CreateAccountAsync()
    {
        ...
        var email = $"backward-api-{userId:N}@example.com";
