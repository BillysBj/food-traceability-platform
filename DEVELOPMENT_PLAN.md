...
- **QLT-009** Quality Read Paths (eingeschoben) — **Roadmap-Status: NOT_STARTED** — M5 hat ausschließlich schreibende Endpunkte gebaut: `quality.read` schützt keinen einzigen Endpunkt, und Probe, Laborergebnis und Sperre sind nach dem Anlegen nicht mehr abrufbar. Umfasst mindestens den Qualitätsstatus in der Lot-Antwort — nach D-53 wird er ständig und von vielen gelesen und steht deshalb unter `lot.read` —, die Proben und Sperren eines Lots, die Ergebnisse einer Probe sowie die anwendbare Spezifikation, deren Grenzwerte nach D-52 ausdrücklich ausgegeben werden, damit ein Labor weiß, wogegen es misst. Blockiert M5 bewusst: „Quality Ready" soll heißen, dass Quality benutzbar ist, nicht nur befüllbar. Zuschnitt beim Schreiben des Tasks.
